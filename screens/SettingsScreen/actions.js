...
export const getDefaultTab = () => async dispatch => {
  ...
  if ( defaulttab_value !== null ) {
    ...
    await AsyncStorage.setItem(
      'defaulttab_value',
      JSON.stringify( INITIAL_STATE.default_tab )
    )
    dispatch( {
      type: CHANGE_DEFAULT_TAB,
...
export const changeAutocompleteSetting = ( toLoad = false ) => async dispatch => {
  ...
  if ( autocomplete_value !== null ) {
    ...
    await AsyncStorage.setItem(
      'autocomplete_value',
      JSON.stringify( INITIAL_STATE.autocomplete )
    )
    dispatch( {
      type: AUTOCOMPLETE_SETTING,
...
export const changeQuackOnRefreshSetting = (
  toLoad = false
) => async dispatch => {
  let quack_on_refresh_value = await AsyncStorage.getItem(
    'quack_on_refresh_value'
  )
  ...
  if ( quack_on_refresh_value !== null ) {
    ...
    await AsyncStorage.setItem(
      'quack_on_refresh_value',
      JSON.stringify( INITIAL_STATE.quack_on_refresh )
    )
    dispatch( {
      type: QUACK_ON_REFRESH_SETTING,
...
export const changeSaveRecentSetting = ( toLoad = false ) => async dispatch => {
  ...
  if ( save_recent_value !== null ) {
    ...
    await AsyncStorage.setItem(
      'save_recent_value',
      JSON.stringify( INITIAL_STATE.save_recent )
    )
    dispatch( {
      type: SAVE_RECENT_SETTING,
